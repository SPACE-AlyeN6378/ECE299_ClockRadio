...
class Display:
    ...
    def text(self, string, col, row, highlight=False):
        if (highlight):
            self.oled.fill_rect(col, row-1, 8*len(string), 9, 1)
            
        text_color = 0 if highlight else 1
        self.oled.text(string, col, row, text_color)

    def box(self, x, y, width, height, fill=False):
        if fill:
            self.oled.fill_rect(x, y, width, height, 1)
        else:
